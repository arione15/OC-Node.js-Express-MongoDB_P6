...
exports.login = (req, res, next) => {
    User.findOne({
            email: req.body.email //query filter (trouver un user dans la BDD dont l'email correspond à ce qui été envoyé dans le body de la requête)
        })
        .then(user => {
            if (!user) {
            ...
            bcrypt.compare(req.body.password, user.password) // si on a trouvé un user avec le bon email alors on compare les mdp
                .then(valid => {
                    if (!valid) {
                        return res.status(401).json({
