...
exports.createSauce = (req, res, next) => {
    ...
    const sauce = new Sauce({ // faire une copie de l'objet sauce et rajouter d'autres paramètres (imageUrl)
        ...
        likes: `${req.protocol}://${req.get('host')}/images/${req.file.filename}`
        dislikes: `${req.protocol}://${req.get('host')}/images/${req.file.filename}`
        usersLiked: `${req.protocol}://${req.get('host')}/images/${req.file.filename}`
        usersDisliked: `${req.protocol}://${req.get('host')}/images/${req.file.filename}`
    });
    sauce.save()
