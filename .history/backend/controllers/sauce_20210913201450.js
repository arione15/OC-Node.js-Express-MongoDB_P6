...
exports.createSauce = (req, res) => {
  ...
  const sauce = new Sauce(
    {...sauceObject, imageUrl: `${req.protocol}://${req.get('host')}/images/${req.file.filename}`, likes: 0, dislikes: 0, usersLiked: [], usersDisliked: [],}); // faire une copie de l'objet sauce et rajouter éventuellement d'autres paramètres
    // aussi rajouter le userId récupéré dans le header Authorization !!!
    // name:
    // manufacturer: ,
    // description: ,
    // mainPepper: ,
  ...
  sauce
    ...
    .then((newSauce) => {
      ...
      if(newSauce.likes !==0){
          likes++;
          usersLiked=usersLiked.push(newSauce.userId);
      }else if(newSauce.dislikes !==0){
        dislikes++;
        usersDisliked=usersDisliked.push(newSauce.userId);
      }
        res.status(201).json({
